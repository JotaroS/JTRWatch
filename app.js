...
getTemp : function(callback){
    ...
    request(url,function (error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
            parseString(body, function (err, result) {
               var max =result.weatherforecast.pref[0].area[3].info[0].temperature[0].range[0]._ ;
                var min =result.weatherforecast.pref[0].area[3].info[0].temperature[0].range[1]._ ;
...
getWeather : function(callback){
    ...
    request(url,function (error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
            parseString(body, function (err, result) {
               var w = result.weatherforecast.pref[0].area[3].info[0].weather[0] + "\n";
               callback(null,w);
